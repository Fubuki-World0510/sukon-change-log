...
| `avatar`         | `<default / guild>` | Lấy Avatar của bạn                                                    |
| `server-info`    |             | Thông tin máy chủ Discord của bạn                                             |
| `web-screenshot`          | `<url>`     | ScreenShot một website mà bạn muốn, để check trước web có gì         |
...
| :-------- | :---------- | :----------------------- |
| `count`   | `<channel>` | Đặt count channel |

| Setups | Option      | Mô tả                    |
| :-------- | :---------- | :----------------------- |
| `jtc`   | `<action>` | Tạo kênh voice "Join to create" |
...
| `banid`          | `<id>`                                                         | Vẫn là ban nhưng bằng ID và có thể ban bên ngoài |
| `excute`          | `X`                                                     | ❌ |
| `mute`           | `<user> <time> <reason>`                                       | Timeout người dùng |
...
| `snipe`          |                                                                | Snipe 1 tin nhắn đã từng bị xóa trước đó, chỉ snipe được 1 tin nhắn đã xóa mới nhất |
| `unban`          | `<userid>`                                                     | Gỡ ban cho user bị ban trong server bằng id user |
| `log-setup`          | `<>`                                                     | Lệnh chưa hoàn thành |
| `voice`          | `<>`                                                     | Lệnh chưa hoàn thành |

| Music        | Option          | Mô tả |
| :----------- | :-------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `autoplay`   |                 | Setup cho Sukon tự động chơi 1 bài mới có nội dung có thể liên quan đến bài bạn vừa nghe, chúng chỉ random thôi, chỉ hỗ trợ Youtube |
| `clearqueue` |                 | Xóa toàn bộ nhạc trong hàng đợi |
| `ctrl`       |                 | Chat làm trôi mất bảng điều khiển nhạc, lệnh này sẽ giúp bạn lấy lại nó |
...
| `lyrics`     | `<song>`        | Lấy lyric nhạc từ Genius |
| `nowplaying` |                 | Hiển thị nhạc hiện tại đang chơi |
| `pause`      |                 | Tạm dừng nhạc |
...
| `queue`      | `<> / <page>`   | Hiển thị tất cả bài hát trong phòng chờ, bạn không cần phải gõ số trang "page" nếu muốn |
...
| `stop`       |                 | Dừng nhạc |
| `volume`     | `<number>`      | Thay đổi âm lượng, max 100% và hơn thế nữa, cẩn thận đau tai |
...
| User     | Option              | Mô tả                                                                                                          |
| :------- | :------------------ | :------------------------------------------------------------------------------------------------------------- |
| `afk`    | `<reason>`          | Đặt bạn vào trạng thái Afk                                                                                                |
| `google` | `<search>`          | Tìm kiếm với Google                                                                                            |
| `bug-report`  | `<issue_image>`     | Trung tâm báo cáo Sukon, khi bạn sử dụng Sukon và gặp phải lỗi, hãy report ngay và nó sẽ gửi đến trung tâm !!! |
| `say`            | `<text> <webhook> <upload>`                                    | Nhờ Sukon nói gì đó |
